...
import { Search, Brain, Zap, Target, TrendingUp, AlertCircle } from 'lucide-react';
...
export default function Research() {
  ...
  const handleAnalyze = () => {
    ...
    const interval = setInterval(() => {
      ...
      setProgress((prev) => Math.min(prev + 15 + Math.random() * 10, 99));
      ...
      if (currentStep >= steps.length) {
        ...
        setTimeout(() => {
          ...
          setResults({
            ...
            quant: 76,
            sentiment: 88,
            narrative: `Based on our AI analysis, ${query.toUpperCase()} presents a strong buying opportunity. The company shows robust fundamental growth with operating margins expanding by 240bps YoY. Quantitative models suggest the stock is undervalued relative to peers. Market sentiment is overwhelmingly positive following recent product announcements.`
          });
        }, 1000);
      }
    }, 1200);
  ...
  return (
    <div className="space-y-6" id="research-page">
      <div className="max-w-4xl mx-auto">
        ...
        <p className="text-[var(--color-text-secondary)] text-center mb-8">Enter a stock symbol to generate a comprehensive AI-driven analysis</p>
        ...
        <Card className="p-2 mb-10 flex items-center bg-white/5 border border-white/20 focus-within:border-[var(--color-primary)] transition-all">
          ...
          <div className="flex items-center space-x-2 px-2 border-l border-white/10 ml-2">
            <select className="bg-transparent text-sm text-gray-300 outline-none border-none cursor-pointer">
              <option className="bg-[#12121a]">Short-term</option>
              <option className="bg-[#12121a]">Long-term</option>
            </select>
          </div>
          <Button 
            ...
            >
              <Card className="p-8 text-center flex flex-col items-center justify-center min-h-[200px]">
                <div className="relative w-16 h-16 mb-6">
                  ...
                  <div 
                    className="absolute inset-0 border-4 border-[var(--color-primary)] rounded-full border-t-transparent animate-spin"
                  ></div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                ...
                <Card className="col-span-1 lg:col-span-2 p-8 flex flex-col justify-center">
                  ...
                  <div className="mt-6 pt-6 border-t border-white/10 flex space-x-4">
                    <Button variant="outline" size="sm"><TrendingUp size={16} className="mr-2" /> Add to Watchlist</Button>
                    <Button variant="secondary" size="sm"><Target size={16} className="mr-2" /> Set Alert</Button>
                  </div>
                </Card>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                  ...
                  <Card key={cluster.label} className="flex flex-col items-center py-6">
                    <h4 className="text-sm text-[var(--color-text-secondary)] uppercase tracking-wider mb-4">{cluster.label}</h4>
