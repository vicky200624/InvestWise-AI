...
class ChatSessionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return ChatRepository.get_sessions_by_user(self.request.user)
...
class ChatMessageView(views.APIView):
    ...
    def post(self, request):
        ...
        if not session_id:
            session = ChatRepository.get_last_session(request.user)
            if not session:
                session = ChatRepository.create_session(request.user, "New Conversation")
...
class VoiceChatView(views.APIView):
    ...
    def post(self, request):
        ...
        session = ChatRepository.get_last_session(request.user)
        if not session:
            session = ChatRepository.create_session(request.user, "Voice Conversation")
